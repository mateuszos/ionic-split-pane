...
export class Animation {
  ...
  private _addProp(state: string, prop: string, val: any): EffectProperty {
    ...
    if (typeof val === 'string' && val.indexOf(' ') < 0) {
      const r = val.match(ANIMATION_CSS_VALUE_REGEX);
  ...
  _progress(stepValue: number) {
    // bread 'n butter
    let val: any;
    const effects = this._fx;
    const nuElements = this._eL;
   ...
   */
  onFinish(callback: Function, onceTimeCallback = false, clearOnFinishCallacks = false): Animation {
   ...
   */
  reverse(shouldReverse = true): Animation {
    const children = this._c;
    for (let i = 0; i < this._cL; i++) {
