...
const menuTypes: { [name: string]: new(...args: any[]) => MenuType } = {};
...
export class MenuController {
  ...
  get(menuId?: string): Menu {
    ...
    return (this.menus.length ? this.menus[0] : null);
  }
   ...
   */
  _register(menu: Menu) {
    assert(this.menus.indexOf(menu) < 0, 'menu was already registered');
  ...
  _unregister(menu: Menu) {
    assert(this.menus.indexOf(menu) >= 0, 'menu is not registered');
    removeArrayItem(this.menus, menu);
  }
