...
export const PLATFORM_CONFIGS: { [key: string]: PlatformConfig } = {
  ...
  'phablet': {
    isMatch(plt: Platform) {
      const smallest = Math.min(plt.width(), plt.height());
      const largest = Math.max(plt.width(), plt.height());
    ...
    settings: {
      activator: function(plt: Platform): string {
        ...
        if (plt.testNavigatorPlatform('linux')) {
          const chromeVersion = plt.matchUserAgentVersion(/Chrome\/(\d+).(\d+)?/);
          if (chromeVersion) {
...
export const PlatformConfigToken = new InjectionToken<{ [key: string]: PlatformConfig }>('PLTCONFIG');
