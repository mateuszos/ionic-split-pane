...
export class Platform {
  ...
  version(): PlatformVersion {
    for (const platformName in this._versions) {
   ...
   */
  registerBackButtonAction(fn: Function, priority = 0): Function {
      ...
      if (win.screen.width > 0 && win.screen.height > 0) {
        if (win['innerWidth'] < win['innerHeight']) {
          ...
          if (this._lW > win['innerWidth']) {
            // Special case: keyboard is open and device is in portraitportrait');
            this._isPortrait = true;
          }
  ...
  init() {
    ...
    for (const platformName in this._registry) {
  ...
  private matchPlatform(platformName: string): PlatformNode {
    ...
    const platformNode = new PlatformNode(this._registry, platformName);
    const rootNode = platformNode.getRoot(this);
...
function insertSuperset(registry: any, platformNode: PlatformNode) {
  const supersetPlaformName = platformNode.superset();
  if (supersetPlaformName) {
    // add a platform in between two exist platforms
    // so we can build the correct hierarchy of active platforms
    const supersetPlatform = new PlatformNode(registry, supersetPlaformName);
...
class PlatformNode {
  ...
  getRoot(plt: Platform): PlatformNode {
    if (this.isMatch(plt)) {
      ...
      const parents = this.getSubsetParents(this.name);
  ...
  getSubsetParents(subsetPlatformName: string): string[] {
    ...
    for (const platformName in this.registry) {
