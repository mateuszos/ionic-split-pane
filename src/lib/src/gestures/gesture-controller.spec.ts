...
describe('gesture controller', () => {
  ...
  it('should initialize without options', () => {
    ...
    const g2 = c.createGesture({ name: 'event2' });
  ...
  it('should test if several gestures can be started', () => {
    const c = new GestureController();
    const g1 = c.createGesture({ name: 'swipe' });
    const g2 = c.createGesture({name: 'swipe1', priority: 3});
    const g3 = c.createGesture({name: 'swipe2', priority: 4});
  ...
  it('should test if several gestures try to capture at the same time', () => {
    const c = new GestureController();
    const g1 = c.createGesture({name: 'swipe1'});
    const g2 = c.createGesture({name: 'swipe2', priority: 2 });
    const g3 = c.createGesture({name: 'swipe3', priority: 3 });
    const g4 = c.createGesture({name: 'swipe4', priority: 4 });
    const g5 = c.createGesture({name: 'swipe5', priority: 5 });
  ...
  describe('BlockerDelegate', () => {
    ...
    it('should disable some events', () => {
      ...
      const goback = c.createGesture({ name: 'goback' });
      expect(goback.canStart()).toEqual(true);

      const g2 = c.createGesture({ name: 'goback2' });
